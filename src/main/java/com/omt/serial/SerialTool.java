    ...
    public static String readFromPort(SerialPort serialPort) throws ReadDataFromSerialPortFailure, SerialPortInputStreamCloseFailure {
        ...
        try {
            ...
            while (true){
                ...
                //System.out.println("lenth---"+bufflenth);
                /*while (bufflenth>0){
                    in.read(bytes);
                    str = byte2hex(bytes);
                    if("aa".equals(str.substring(1,3))){
                        //bts = bytes;
                ...
                if(bufflenth>0){
                    in.read(bytes);
                    str += byte2hex(bytes).trim()+",";
                    num++;

                    //System.out.println(byte2hex(bytes));
                    //System.out.println("str----"+str.trim().substring(0,str.length()));
                    if(num==21) {
                        str = str.trim().substring(0, str.length());
                        return str;
                    }
                }
            }
